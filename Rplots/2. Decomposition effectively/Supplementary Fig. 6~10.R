...
load(paste0(workdir, "Supplementary Fig. 6~10_source_data1.Rdata"))
init_sts_self_list = sts_self_list
init_sts_prop_list = sts_prop_list
load(paste0(workdir, "Supplementary Fig. 6~10_source_data2.Rdata"))
...
for (d in datlist) {
  ...
  for (t in unique(t_df$Type)) {
    ...
    percentValues = round(t_df_$Values, 2)*100
    ggplot(t_df_, aes(x = Order, y = Values, fill = Method)) +
      ...
      scale_fill_manual(labels = c(expression(I-clusters~(p["t,i"])),
                                   expression(I-clusters~(q["t,i"])),
                                   expression(M-clusters~(p["t,i"])),
                                   expression(M-clusters~(q["t,i"]))), 
      ...
      facet_wrap(~Type)
    ggsave(path = workdir, 
           filename = paste0("Supplementary Fig. 6~10_",d,"_", t, ".png"),
