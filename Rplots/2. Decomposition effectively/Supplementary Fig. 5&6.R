...
load(paste0(workdir, "Supplementary Fig. 5&6_source_data1.Rdata"))
init_sts_self_list = sts_self_list
init_sts_prop_list = sts_prop_list
load(paste0(workdir, "Supplementary Fig. 5&6_source_data2.Rdata"))
...
ggplot(t_df, aes(x = Order, y = Values, fill = Method)) +
  geom_bar(position="dodge", stat="identity")+
  theme_bw() +
  labs(y = "Proportion", x = "Rank") +
  ...
  facet_wrap(~Type)
ggsave(path = workdir, filename = "Supplementary Fig. 5.png",width = 24, height = 18, device='png', dpi=300)


t_df = df[which(df$Method %in% c("Init_prop", "M_prop")),]
ggplot(t_df, aes(x = Order, y = Values, fill = Method)) +
  geom_bar(position="dodge", stat="identity")+
  theme_bw() +
  labs(y = "Proportion", x = "Rank") +
  ...
  theme(text=element_text(size=20,  family="serif",face = 'bold'),
        ...
        panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  facet_wrap(~Type)
ggsave(path = workdir, filename = "Supplementary Fig. 6.png",width = 24, height = 18, device='png', dpi=300)
