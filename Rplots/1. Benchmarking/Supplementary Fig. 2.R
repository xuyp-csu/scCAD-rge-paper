...
ggplot(data_melt, aes(x=Method, y=Ranking, fill=Dataset, color=Dataset)) + 
  ...
  xlab("") +
  ylab(bquote(Rank~of~Methods~Based~on~Acc[rare~cell~type]~Across~Datasets)) +
...
ggplot(data_melt, aes(x=Method, y=Ranking, fill=Dataset, color=Dataset)) + 
  ...
  theme(panel.grid.major = element_line(colour="gray"),
        ...
        legend.text = element_text(size = 13, family = "serif"))+
  guides(fill = guide_legend(nrow = 3))
ggsave(path = workdir, filename = "Supplementary Fig. 2e.png",width = 12, height = 8, device='png', dpi=300)
