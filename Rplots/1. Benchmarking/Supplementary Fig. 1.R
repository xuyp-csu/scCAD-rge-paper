...
df = read_excel(paste0(workdir, "Supplementary Fig. 1_source_data4.xlsx"))
df = as.data.frame(df)
rownames(df) = df[,1]
df = df[,-1]
df = melt(df, measure.vars = colnames(df))
colnames(df) = c("med","acc")
...
ggplot(df, aes(x = med, y = acc, fill = med)) +
  geom_boxplot(width = 0.5, outlier.shape = NA)+
  theme_bw() +
  ylab(bquote(Acc[rare~cell~type]))  +
...
ggplot(df, aes(x = med, y = MCC, fill = med)) +
  ...
  theme(text=element_text(size=20,  family="serif",face = 'bold'),
        ...
        axis.title.x=element_blank(), 
        panel.grid.major = element_blank(), panel.grid.minor = element_blank())
ggsave(path = workdir, filename = "Supplementary Fig. 1d.png",width = 14, height = 6, device='png', dpi=300)
