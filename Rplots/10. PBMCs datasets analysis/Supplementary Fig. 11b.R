...
load(paste0(workdir, "Supplementary Fig. 11b_source_data1_PBMC-bench-1_umap.Rdata"))
...
ggsave(path = workdir, filename = "Supplementary Fig. 11b.png",width = 12, height = 6, device='png', dpi=300)
...
ggplot(temp_umap, aes(x=UMAP_1, y=UMAP_2, color=label)) + 
  ...
  theme(text=element_text(size=15,  family="serif",face = 'bold'),
        ...
        panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  guides(color = guide_legend(override.aes = list(size = 4)))
ggsave(path = workdir, filename = "Supplementary Fig. 11b_Rlab.png",width = 8, height = 6, device='png', dpi=300)
