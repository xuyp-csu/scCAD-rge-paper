...
scCAD_res = read.table(paste0(workdir, "Supplementary Fig. 17a_source_data2_PBMC-bench-3_scCAD_res.txt"))[,1]
lab = read.table(paste0(workdir, "Supplementary Fig. 17a_source_data3_PBMC-bench-3_lab.txt"))[,1]
load(paste0(workdir, "Supplementary Fig. 17c_source_data1_PBMC-bench-3_degs.Rdata"))
load(paste0(workdir, "Supplementary Fig. 17c_source_data2_PBMC-bench-3_subdata.Rdata"))
...
png(paste0(workdir, "Supplementary Fig. 17c_R7.png"), width = 8, height = 12, units = 'in', res = 600)
...
load(paste0(workdir, "Supplementary Fig. 17c_source_data3_PBMC-bench-3_subdata.Rdata"))
...
png(paste0(workdir, "Supplementary Fig. 17c_R8.png"), width = 8, height = 12, units = 'in', res = 600)
pheatmap(sub_dat[,idx], border_color = NA, cluster_cols = FALSE, 
         annotation_colors = ann_colors,
         annotation_col = annotation, show_colnames = FALSE, annotation_names_col = TRUE)
dev.off()
