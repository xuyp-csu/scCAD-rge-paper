...
load(paste0(workdir, "Supplementary Fig. 19a_source_data1_PBMC-bench-3_tsne.Rdata"))
...
ggsave(path = workdir, filename = "Supplementary Fig. 19a.png",width = 8, height = 6, device='png', dpi=300)
...
ggplot(temp_tsne_out, aes(x=tSNE_1, y=tSNE_2, color=label)) + 
  ...
  theme(text=element_text(size=15,  family="serif",face = 'bold'),
        ...
        panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  guides(color = guide_legend(override.aes = list(size = 4)))
ggsave(path = workdir, filename = "Supplementary Fig. 19a_Rlab.png",width = 8, height = 6, device='png', dpi=300)
