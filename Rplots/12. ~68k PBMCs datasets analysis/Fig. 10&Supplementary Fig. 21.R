...
tsne_out = read.table(paste0(workdir, "Fig. 10_source_data1_tsne.tsv"),
                      header = T,sep = "\t")
df = tsne_out[,c(1,2)]
...
ggsave(path = workdir, filename = "Fig. 10a.pdf",width = 70, height = 40, units = "mm",
       dpi=300)

##################### rare cell type ###########################

lines <- readLines(paste0(workdir, "Fig. 10_source_data2_scCAD_res.txt"))
rare_cell_id = c()
for (l in lines) {
...
ggplot(df, aes(x=tSNE_1, y=tSNE_2, color=Rlab)) + 
  ...
  theme(text=element_blank(),
        ...
        panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  guides(color = guide_legend(ncol =1, override.aes = list(size = 1)))
ggsave(path = workdir, filename = "Fig. 10b.pdf",width = 52, height = 40, units = "mm",
       dpi=300)

##################### pearson heatmap ###########################

ref_genes = read.csv(paste0(workdir, "Fig. 10_source_data3_ref_gene.csv"))
load(paste0(workdir, "Fig. 10_source_data4_subdata.Rdata"))
load(paste0(workdir, "Fig. 10_source_data5_refdata.Rdata"))
lines <- readLines(paste0(workdir, "Fig. 10_source_data2_scCAD_res.txt"))
rare_cell_id = c()
for (l in lines) {
  rare_cell_id = c(rare_cell_id, list(strsplit(l,"\t")[[1]]))
...
png(paste0(workdir, "Fig. 10c.png"), width = 60, height = 60, units = 'mm', res = 600)
pheatmap(sim, annotation_col = annotation, annotation_row = annotation, 
         ...
         fontsize = 5, 
         treeheight_row = 1, treeheight_col = 1, legend = F,
...
ggplot(df, aes(factor(gene), count, fill = gene)) +
  geom_violin(scale = "width", adjust = 1, trim = TRUE) +
  scale_y_continuous(expand = c(0, 0), position="right", labels = function(x)
    c(rep(x = "", times = length(x)-2), x[length(x) - 1], "")) +
  facet_grid(rows = vars(lab), scales = "free", switch = "y") +
  theme_cowplot(font_size = 5,  font_family = "sans") +
  theme(legend.position = "none", panel.spacing = unit(0, "lines"),
        plot.title = element_text(size=5,  family="sans", hjust = 0.5),
        panel.background = element_rect(fill = NA, color = "black"),
        strip.background = element_blank(),
        strip.text = element_text(size=5,  family="sans"),
        strip.text.y.left = element_text(size=5,  family="sans", angle = 0),
        axis.text.x = element_text(size=5,  family="sans", angle = 45, hjust = 1, vjust = 0.5),
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        axis.ticks.x=element_blank(), 
        axis.ticks.y=element_blank()) +
  ggtitle("") + xlab("") + ylab("Expression Level")

ggsave(path = workdir, filename = "Fig. 10d.pdf",width = 160, height = 50, units = "mm",
       dpi=300)


##################### expression heatmap ###########################

load(paste0(workdir, "Supplementary Fig. 21_source_data1_degs.Rdata"))
load(paste0(workdir, "Supplementary Fig. 21_source_data2_subdata.Rdata"))
lines <- readLines(paste0(workdir, "Fig. 10_source_data2_scCAD_res.txt"))
rare_cell_id = c()
for (l in lines) {
  rare_cell_id = c(rare_cell_id, list(strsplit(l,"\t")[[1]]))
...
png(paste0(workdir, "Supplementary Fig. 21a.png"), width = 8, height = 16, units = 'in', res = 600)
...
png(paste0(workdir, "Supplementary Fig. 21b.png"), width = 8, height = 16, units = 'in', res = 600)
