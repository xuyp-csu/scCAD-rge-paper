...
load(paste0(workdir, "Fig. 8a_8b_8c_source_data1_Kidney_normal_umap.Rdata"))
lab = read.table(paste0(workdir, "Fig. 8a_8b_8c_source_data2_Kidney_normal_lab.txt"))[,1]
...
ggsave(path = workdir, filename = "Fig. 8a.pdf",width = 60, height = 50, units = "mm",
       dpi=300)
...
ggplot(temp_umap, aes(x=umap_1, y=umap_2, color=label)) + 
  ...
  theme(text=element_blank(),
        ...
        panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  guides(color = guide_legend(ncol =1, override.aes = list(size = 1)))
ggsave(path = workdir, filename = "Fig. 8b_Rlab.pdf",width = 60, height = 50, units = "mm",
       dpi=300)

scCAD_res = read.table(paste0(workdir, "Fig. 8a_8b_8c_source_data3_Kidney_normal_scCAD_res.txt"))[,1]
temp_umap = umap
...
ggplot(temp_umap, aes(x=umap_1, y=umap_2, color=label)) + 
  ...
  theme(text=element_blank(),
        ...
        panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  guides(color = guide_legend(ncol =1, override.aes = list(size = 1)))
ggsave(path = workdir, filename = "Fig. 8c_scCAD.pdf",width = 60, height = 50, units = "mm",
       dpi=300)
